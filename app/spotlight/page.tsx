...
export default async function SpotlightPage() {
  try {
    ...
    // 2. Fetch specific massive data structures for this game — allSettled to prevent crash
    const [newsResult, streamsResult, guidesResult] = await Promise.allSettled([
    ...
    console.error("Spotlight page critical error:", err);
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        <div className="w-20 h-20 bg-surface-2/30 rounded-full flex items-center justify-center border border-surface-2">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#EAB308]"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
        </div>
        <h2 className="text-2xl font-bold text-white">Radar temporalmente offline</h2>
        <p className="text-text-secondary text-center max-w-md">Los servidores de datos externos no responden. Inténtalo de nuevo en unos segundos.</p>
        <a href="/spotlight" className="px-6 py-3 rounded-xl bg-[#EAB308] text-black font-bold uppercase text-sm hover:scale-105 transition-transform">
          Reintentar ⚡
        </a>
      </div>
    );
